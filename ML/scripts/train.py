...
Usage:
    python scripts/train.py \
        ...
        --composite-pv-weight 0.5 \
        --patience 20 \
        --source-weight realworld=5

Resume after a crash / pause:
    python scripts/train.py --ckpt-dir <same dir> --resume

The `--resume` flag loads `<ckpt-dir>/latest.pt`, which is rewritten at the end
of every epoch with the full training state (model + optimizer + scheduler +
scaler + EMA + stage + epoch + best_scores + patience_ctr). EMA weights are
saved separately as `ema-best.pt` whenever the EMA model's composite score
exceeds the EMA running best.

Class-weighted CrossEntropyLoss: weights derived from train-split class
inverse frequency, capped at `--class-weight-cap` (default 3.0×) and
re-normalized to mean 1.0. Disable with `--class-weight-cap 1.0`.
"""
...
def _save_latest(ckpt_path: Path, *, model, optimizer, scheduler, scaler, ema,
                 ...
                 patience_ctr: int, arch: str) -> None:
    """Atomic write of the full training state to latest.pt."""
    state = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Class-weighted loss for the v7 collapse-recovery (mosaic_virus, TYLCV, corn_rust).
    ...
    for epoch in range(start_s2_epoch, cfg.stage2_epochs):
        ...
        # EMA evaluation (separate pass on the EMA-weighted model).
