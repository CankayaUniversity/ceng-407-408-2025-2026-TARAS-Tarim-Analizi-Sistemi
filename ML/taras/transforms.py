...
def build_train_transform(img_size: int = IMG_SIZE, field_domain_rand: bool = True) -> A.Compose:
    """Train-time augmentation. When `field_domain_rand=True` (default for v7),
    layers in heavier augmentations specifically targeting the lab→field
    domain shift seen in PlantDoc photos: JPEG compression artifacts, varied
    lighting/shadows, motion blur, simulated phone-camera focus issues."""
    pipeline = [
    ...
    if field_domain_rand:
        # v7: heavier ColorJitter + JPEG compression + shadow simulation +
        # heavier blur. Targets the lab→field domain shift specifically.
        pipeline += [
            A.ColorJitter(brightness=0.25, contrast=0.25, saturation=0.25, hue=0.03, p=0.7),
            A.OneOf([
                A.MotionBlur(blur_limit=15),
                A.GaussianBlur(blur_limit=15),
