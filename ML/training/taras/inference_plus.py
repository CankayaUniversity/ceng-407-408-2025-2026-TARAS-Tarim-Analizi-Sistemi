...
Backend-agnostic: takes either a PyTorch `nn.Module` or an
`onnxruntime.InferenceSession`. Lambda code pulls `softmax_with_temperature`,
`predict_with_tta`, and `classify_with_uncertainty` from here.

Inputs are batched NCHW float32 arrays [1, 3, H, W], ImageNet-normalised;
model output is logits [1, num_classes].
"""
