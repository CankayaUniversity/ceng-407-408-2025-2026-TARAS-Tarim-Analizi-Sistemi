...
Intent (see handoff §6):
  - Orientation-invariant leaves → H- and V-flip both on
  - Lesion colour is diagnostic → hue perturbation kept at 0.02
  - Field realism → blur/noise at low p, coarse dropout for background-bias
    resistance (Noyan 2022)
"""
...
def build_train_transform(img_size: int = IMG_SIZE, field_domain_rand: bool = True) -> A.Compose:
    """Train-time augmentation. `field_domain_rand=True` adds heavier aug
    (JPEG artifacts, shadows, motion blur) for the lab→field domain shift.
    """
    pipeline = [
