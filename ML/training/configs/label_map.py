...
Cross-crop merging where pathology + treatment is identical (bacterial_spot
across tomato/pepper/peach, late_blight on tomato/potato, etc.).

Sources merged: PlantVillage (`Crop___Disease`), PlantDoc (`Crop Disease leaf`),
and Mustafa's merged/train (`Crop_Disease`; his Potato_Leaf_blight bundles
early+late, disambiguated by filename in the v6 pool builder).

CLASS_NAMES is alphabetical to match `Backend/Lambda_v5/labels.py::CLASS_NAMES`.

Excluded from this round (no PlantDoc field validation, scheduled for v7):
apple_scab, black_rot, cedar_apple_rust, esca, leaf_blight_grape,
haunglongbing, leaf_scorch.
"""
