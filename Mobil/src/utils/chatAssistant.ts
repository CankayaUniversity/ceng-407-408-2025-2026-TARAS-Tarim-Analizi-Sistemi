// Chat asistani - anahtar kelimeye gore ekran yonlendirme
// getAssistantResponse(input, setScreen) ile yanit alinir

import { SCREEN_TYPE, ScreenType } from "../constants";

const RESPONSES: Record<string, { screen?: ScreenType; reply: string }> = {
  home: {
    screen: SCREEN_TYPE.HOME,
    reply:
      "Ana Sayfa ekranına yönlendirdim. 3D düzlemi döndürebilir, rengi değiştirmek için dokunabilirsiniz.",
  },
  "ana sayfa": {
    screen: SCREEN_TYPE.HOME,
    reply:
  ...
  hastalık: {
    screen: SCREEN_TYPE.DISEASE,
    reply:
      "Hastalık Tespiti ekranına yönlendirdim. Bitkileri taramak için kamerayı kullanabilirsiniz.",
  },
  disease: {
    screen: SCREEN_TYPE.DISEASE,
    reply:
  ...
  ayarlar: {
    screen: SCREEN_TYPE.SETTINGS,
    reply:
      "Ayarlar ekranına yönlendirdim. Tema ayarlarınızı yapabilir ve çıkış yapabilirsiniz.",
  },
  settings: {
    screen: SCREEN_TYPE.SETTINGS,
    reply:
  ...
  çizelge: {
    screen: SCREEN_TYPE.TIMETABLE,
    reply:
      "Çizelge ekranına yönlendirdim. Sensör verilerini buradan takip edebilirsiniz.",
  },
  timetable: {
    screen: SCREEN_TYPE.TIMETABLE,
    reply:
