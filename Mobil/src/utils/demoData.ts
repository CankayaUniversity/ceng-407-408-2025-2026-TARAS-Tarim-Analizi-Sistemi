// Demo data for testing - will be replaced with real API data
...
const DEMO_SHAPES: PolygonGenerator[] = [
  () => ({ exterior: [[0, 0], [100, 0], [100, 60], [0, 60]] }),
  () => ({ exterior: [[0, 0], [80, 0], [80, 40], [50, 40], [50, 70], [0, 70]] }),
  () => ({ exterior: [[10, 5], [95, 0], [100, 55], [5, 65]] }),
  () => ({ exterior: [[50, 0], [100, 35], [80, 90], [20, 90], [0, 35]] }),
  () => ({
    exterior: [[0, 0], [120, 0], [120, 80], [0, 80]],
    holes: [[[70, 45], [100, 45], [100, 65], [70, 65]]],
  }),
  () => ({ exterior: [[20, 0], [80, 0], [100, 60], [0, 60]] }),
  () => ({ exterior: [[30, 0], [70, 0], [100, 30], [100, 60], [70, 90], [30, 90], [0, 60], [0, 30]] }),
  () => ({ exterior: [[0, 0], [5, 0], [5, 5], [0, 5]] }),
  () => ({ exterior: [[0, 0], [1000, 0], [1000, 600], [0, 600]] }),
];
...
function isPointInPolygon(x: number, z: number, polygon: [number, number][]): boolean {
  let inside = false;
...
function isPointInHoles(x: number, z: number, holes?: [number, number][][]): boolean {
...
function randomPointInPolygon(
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    const x = paddedBounds.minX + rng() * (paddedBounds.maxX - paddedBounds.minX);
    const z = paddedBounds.minZ + rng() * (paddedBounds.maxZ - paddedBounds.minZ);

    if (isPointInPolygon(x, z, polygon.exterior) && !isPointInHoles(x, z, polygon.holes)) {
      return { x, z };
    }
  }
...
export function generateDemoFieldData(seed?: number, shapeIndex?: number): FieldData {
...
export function getDemoShape(name: 'rectangle' | 'l-shape' | 'irregular' | 'pentagon' | 'with-hole' | 'trapezoid' | 'hexagon' | 'tiny' | 'huge'): FieldPolygon {
...
const DEMO_FIELDS_CONFIG: Array<{ id: string; name: string; area: number; shapeIndex: number }> = [
...
export function generateDemoDashboardData(fieldId: string): DashboardData {
  const fieldConfig = DEMO_FIELDS_CONFIG.find(f => f.id === fieldId) ?? DEMO_FIELDS_CONFIG[0];

  const seed = fieldId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) * 1000;
  const fieldData = generateDemoFieldData(seed, fieldConfig.shapeIndex);

  const avgMoisture = fieldData.nodes.length > 0
    ? Math.round(fieldData.nodes.reduce((sum, n) => sum + n.moisture, 0) / fieldData.nodes.length)
    : 50;

  const avgTemp = fieldData.nodes.length > 0
    ? Math.round(fieldData.nodes.reduce((sum, n) => sum + n.airTemperature, 0) / fieldData.nodes.length * 10) / 10
    : 25;

  const avgHumidity = fieldData.nodes.length > 0
    ? Math.round(fieldData.nodes.reduce((sum, n) => sum + n.airHumidity, 0) / fieldData.nodes.length)
    : 60;
